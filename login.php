...
if(isset($_POST['signin-submit'])) {
    $user_name_sign_in = trim($_POST['user_name_sign_in']);
    $user_name_sign_in = strip_tags($user_name_sign_in);
    $user_name_sign_in = htmlspecialchars($user_name_sign_in);
    ...
    if(empty($user_name_sign_in)) {
      $error = true;
      $user_name_sign_inError = "Please enter your user name!";
    }
    ...
    if(!$error) {
      ...
      $res = mysqli_query($conn, "SELECT `PK_user_id`,`user_name`,email,pwd FROM users WHERE email='$email_sign_in'");
